...
export default class Environment {
  ...
  projectPath () {
    ...
    if (currentEditor === undefined) return projectPaths[0]

    const currentEditorPath = currentEditor.getPath()
    const currentProject = projectPaths.find((p) => currentEditorPath.startsWith(p))
    return currentProject || projectPaths[0]
  }
