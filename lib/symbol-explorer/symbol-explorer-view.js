...
import Global from '../autocompletion/definitions/global'
const DEFAULT_STATE = {
  files: {},
  keyword: null,
  currentEditor: null
...
export default class TerminalView {
  ...
  render () {
    return <div class='agk-symbol-explorer'>
      ...
      <div class='agk-scroller'>
        {this.getFiles().map(({ file, functions, types, constants, globals }) => {
            ...
            <ul class='agk-select-list'>
              {constants.map((constant) => {
                return <li on={{ click: () => this.handleDefinitionClicked(constant) }}>
                  ...
                  <span class="agk-monospace agk-subtle agk-ml-half">{constant.value}</span>
                </li>
              })}
              {types.map((type) => {
              ...
              {globals.map((global) => {
                return <li on={{ click: () => this.handleDefinitionClicked(global) }}>
                  <span class='agk-select-badge red'>g</span>
  ...
  getFiles () {
    return Object
      ...
      .map((file) => {
        ...
        return {
          file,
          functions: this.getFunctions(definitions),
          types: this.getTypes(definitions),
          constants: this.getConstants(definitions),
          globals: this.getGlobals(definitions)
        }
      })
      .filter(({ functions, types, constants, globals }) => functions.length > 0 || types.length > 0 || constants.length > 0 || globals.length > 0)
  }
  ...
  getGlobals (definitions) {
    return definitions
      .filter((definition) => {
        if (this.state.keyword === null) return definition instanceof Global

        return definition instanceof Global && this.state.keyword.test(definition.name)
      })
      .sort((a, b) => a.name.localeCompare(b.name))
  }
