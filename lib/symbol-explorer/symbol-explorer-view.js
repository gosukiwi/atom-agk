...
export default class TerminalView {
  ...
  render () {
    return <div class='agk-symbol-explorer'>
      ...
      {this.getFiles().map(({ file, functions, types }) => {
        ...
        return <div>
          ...
          <ul class='agk-select-list'>
            {types.map((type) => {
              return <li>
                <span class='agk-select-badge-yellow'>f</span>
                <span>{type.name}</span>
              </li>
            })}
            {functions.map((func) => {
              return <li on={{ click: () => this.handleDefinitionClicked(func) }}>
                <span class='agk-select-badge-blue'>t</span>
  ...
  getFiles () {
    return Object
      ...
      .map((file) => {
        ...
        return { file, functions: this.getFunctions(definitions), types: this.getTypes(definitions) }
      })
      .filter(({ functions, types }) => functions.length > 0 || types.length > 0)
  }
