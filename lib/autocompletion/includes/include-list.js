...
export default class IncludeList {
  ...
  findAllBySource (sourcefile) {
    return this.includes.filter((include) => include.sources.includes(sourcefile))
  }

  find (sourcefile, file) {
    return this.includes.find((include) => include.file === file && include.sources.includes(sourcefile))
  }
