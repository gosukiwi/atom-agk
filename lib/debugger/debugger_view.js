...
export default class DebuggerView {
  constructor (serializedState) {
    ...
    this.element.innerHTML = `
      <div class="agk-debugger__controls">
        <div class='btn-group'>
          <button class='btn stop'><span class="icon icon-primitive-square"></span> <span class="label">Stop</span></button>
          <button class='btn continue'><span class="icon icon-playback-play"></span> <span class="label">Continue</span></button>
        </div>
      </div>
    ...
    this.element.querySelector('.btn.stop').addEventListener('click', () => {
      this.emitter.emit('stop-pressed')
    })
  ...
  onStopPressed (cb) {
    return this.emitter.on('stop-pressed', cb)
  }
