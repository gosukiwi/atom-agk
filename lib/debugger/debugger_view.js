  ...
  render () {
    this.element.innerHTML = `
      <div class="agk-debugger__controls">
        <div class='btn-group'>
          <button class='btn toggle'><span class="icon ${this.state.started ? 'icon-primitive-square' : 'icon-playback-play'}"></span> ${this.state.started ? 'Stop' : 'Start'}</button>
          ${this.state.started
            ? '<button class="btn continue"><span class="icon icon-playback-fast-forward"></span> Continue</button>'
            : ''}
        </div>
      </div>
