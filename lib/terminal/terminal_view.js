...
export default class TerminalView {
  ...
  render () {
    return <div class="agk-debugger">
      ...
      <div class="agk-debugger__output" ref='output'>
        <ul>
          {this.state.console.map((line) => <li class={ `indentation-${line.indentation}` }>{this.formatLine(line.line)}</li>)}
        </ul>
      </div>
  ...
  formatLine (text) {
    ...
    if (text.startsWith('< error:')) {
      return <span class="agk-debugger__error">
        <span class='badge badge-small badge-error'>error</span>
        {text.substring(8).trim()}
      </span>
    }

    const match = text.match(/(?<file>[^\s]+\.agc):(?<line>\d+): error:(?<rest>.*)/)
    if (match === null) return text

    const { file, line, rest } = match.groups
    return <span class="agk-debugger__error">
      <span class='badge badge-small badge-error'>error</span>
      <a class="agk-debugger__link" on={{ click: () => this.goToError(file, line) }}>{file}:{line}</a>
      : {rest}
    </span>
  }
  ...
  goToError (file, line) {
    ...
    const filepath = env.join(env.projectPath(), file)
    atom.workspace.open(filepath, { initialLine: line - 1 })
  }
}
