  ...
  render () {
    return <div class="agk-debugger">
      <div class="agk-debugger__controls">
        <div class='btn-group'>
          <button on={{ click: this.handleToggleButtonClicked }} class='btn'><span class={`icon ${this.state.started ? 'icon-primitive-square' : 'icon-playback-play'}`}></span>{this.state.started ? 'Stop Debugger' : 'Start Debugger'}</button>
