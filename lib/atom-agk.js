...
import BreakpointManager from './debugger/breakpoint_manager'
...
export default {
  ...
  activate (state) {
    ...
    // initialize debugger
    const breakpoints = new BreakpointManager(this.subscriptions)
    const runner = new Runner(breakpoints)
    this.debugger = new Debugger(this.subscriptions, runner)
