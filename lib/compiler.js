...
export default class Compiler {
    ...
    this.process.onClose((res) => {
      if (res === 0) {
        ...
        this.terminal.write(<span>
          <span class="badge badge-small badge-success">success</span> Compilation successful
        </span>)
      }
    })
  ...
  goToFirstError (output) {
    ...
    const path = this.env.join(this.env.projectPath(), file)
    if (!this.env.exists(path)) return

    atom.workspace.open(path, { initialLine: line - 1 })
  }
}
